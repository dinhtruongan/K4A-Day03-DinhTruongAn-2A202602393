...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Observation luôn quay lại LLM để LLM quyết định bước tiếp theo
            # hoặc trả về type == "text" ở vòng lặp kế tiếp.
            current_prompt = (
                f"Yêu cầu ban đầu: {user_query}\n"
                f"Observation từ tool {tool_name}: {json.dumps(obs_data, ensure_ascii=False)}\n"
                "Hãy suy luận bước tiếp theo. Nếu đã đủ dữ liệu, trả về câu trả lời cuối cùng bằng text."
            )
            continue
